...
/// Shift cap for the bounded backoff applied to retained unresolved inflight
/// commands: the poll interval doubles per over-budget retry up to 16x the
/// configured inflight threshold.
const INFLIGHT_RETAINED_BACKOFF_MAX_SHIFT: u32 = 4;
...
pub struct InflightCheckResult {
    /// Events resolving commands that reached the retry budget from positive venue evidence.
    pub events: Vec<OrderEventAny>,
...
pub struct ExecutionManagerConfig {
    ...
    /// Maximum number of retries for inflight checks; at the budget a pending
    /// command is resolved from the latest venue status, or retained and polled
    /// with bounded backoff when no positive venue evidence exists.
...
struct InflightCheck {
    ...
    // Latest venue status observed while the command remained unresolved. Only
    // written for reports received after registration that preserve inflight
    // tracking; read at the retry budget as the sole positive evidence that a
    // cancel/modify attempt did not take effect.
    pub last_venue_status: Option<OrderStatus>,
}
...
impl ExecutionManager {
    ...
    /// the venue for the order's current status. At the retry budget, the latest observed
    /// venue status decides: an unchanged `Accepted` snapshot resolves the pending command
    /// as `CancelRejected`/`ModifyRejected` (the command did not take effect), while a
    /// command without positive venue evidence is retained unresolved and polled with
    /// bounded backoff. The passage of time alone never produces a terminal event.
    pub fn check_inflight_orders(&mut self) -> InflightCheckResult {
        ...
        for client_order_id in to_check {
            ...
            let (retry_count, last_venue_status) = {
                let Some(check) = self.inflight_checks.get_mut(&client_order_id) else {
                    continue;
                };

                // The poll interval doubles for every retry beyond the budget so
                // a command the venue never answers backs off instead of being
                // terminalized; below the budget the cadence stays `threshold`.
                let next_retry = check.retry_count.saturating_add(1);
                let excess = next_retry.saturating_sub(self.config.inflight_max_retries);
                let interval = threshold
                    .saturating_mul(1u32 << excess.min(INFLIGHT_RETAINED_BACKOFF_MAX_SHIFT));
                ...
                check.retry_count = next_retry;
                check.last_query_at = Some(now);
                (check.retry_count, check.last_venue_status)
            };
            self.order_query_recency.mark(client_order_id);
            self.recon_check_retries
                .insert(client_order_id, retry_count);

            if retry_count >= self.config.inflight_max_retries {
                ...
                match order.status() {
                    OrderStatus::PendingCancel | OrderStatus::PendingUpdate
                        if last_venue_status == Some(OrderStatus::Accepted) =>
                    {
                        // The freshest venue report still shows the order live:
                        // the pending command did not take effect, so resolve it
                        // as a rejection and restore the pre-command state
                        // instead of fabricating a terminal outcome.
                        let reason = Ustr::from("INFLIGHT_TIMEOUT");
                        let event = if order.status() == OrderStatus::PendingCancel {
                    ...
                    | OrderStatus::Submitted => {
                        // No positive venue evidence yet: the command stays
                        // unresolved and keeps polling on the backoff interval.
                        // Only a venue terminal report (or the bounded
                        // missing-order ladder) may conclude it.
                        if retry_count == self.config.inflight_max_retries.max(1) {
                            log::warn!(
                                "Order {client_order_id} still {} after {retry_count} inflight checks without authoritative venue status: retaining unresolved and polling with backoff",
                                order.status(),
                            );
                        }
                    }
            ...
            if let Some(order) = self.get_order(client_order_id) {
                // Query the venue for the current order status: intermediate
                // retries poll for the report that resolves the command, and
                // retained unresolved commands keep polling on backoff.
                let ts_now = self.clock.borrow().timestamp_ns();
